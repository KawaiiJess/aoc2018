...
public class One
{
    ...
    {
        String in = "[1518-06-05 00:46] falls asleep\n" +
        ...
        String sample = "[1518-11-01 00:00] Guard #10 begins shift\n" +
                "[1518-11-01 00:05] falls asleep\n" +
                "[1518-11-01 00:25] wakes up\n" +
                "[1518-11-01 00:30] falls asleep\n" +
                "[1518-11-01 02:55] wakes up\n" +
        ...
        List<String> entries = Arrays.asList(in.split("\n"));
        //List<String> entries = Arrays.asList(sample.split("\n"));
        entries.sort(new compareDate());

        //int[][] info = new int[100][60];
        int[][] info = new int[4000][60];
        ...
        while (index < entries.size())
        {
            ...
            Scanner getNum = new Scanner(sub);
            int guard = getNum.nextInt();
            index++;

            do
            {
                if (entries.get(index).contains("Guard"))
                {
                    break; //Fucking guard #929 didn't sleep
                }

                int[] sleep = parseDate(entries.get(index));
                int sleepMin = sleep[3];
                index++;
                int[] wake = parseDate(entries.get(index));
                int wakeMin = wake[3];

                while (sleepMin != wakeMin)
                {
                    info[guard][sleepMin % 60]++;
                    sleepMin++;
                }
                index++;
            } while (index < entries.size() && !entries.get(index).contains("Guard"));
        }
        ...
        System.out.println(guard * maxIndex);
    }

    private static int[] parseDate(String line)
    {
        int[] date = new int[4];
        date[0] = Integer.parseInt(line.substring(1, 5));
        date[1] = Integer.parseInt(line.substring(6, 8));
        date[2] = Integer.parseInt(line.substring(9, 11));
        ...
        //date[3] = Integer.parseInt(line.substring(12, 14));
        //date[4] = Integer.parseInt(line.substring(15, 17));

        return date;
    }

    //Defines a comparator class comparing by box ID.
    private static class compareDate implements Comparator<String>
    {
        public int compare(final String a, final String b)
        {
            int[] aDate = parseDate(a);
            int[] bDate = parseDate(b);
